...
def make_snake():
    ...
    for i in range(5):
        x_snake+=50
        listsq.append(canvas.create_oval(x_snake,y_snake,x_snake+50,y_snake+50,fill='black'))
...
def screenEnd():
    ...
    x_snake = 18 if x_snake<18 else x_snake
    x_snake = 1468 if x_snake>1468 else x_snake
    y_snake = 7 if y_snake<7 else y_snake
    y_snake = 807 if y_snake>807 else y_snake
...
def update(x1,y1):
    ...
    if (x1+25) in range(x_mouse,x_mouse+51) and (y1+25) in range(y_mouse,y_mouse+51):        
        ...
        listsq.pop(0)
        listsq.append(canvas.create_oval(x1,y1,x1+50,y1+50,fill='black'))
        listpos.pop(0)
